...
// Each section data file imports from here — keep this file stable.
...
export interface ListeningQuestion {
  ...
  type: LQType;
  q: string;
  opts?: string[];   // MCQ: full text e.g. "A. Monday"
  answer: string;    // MCQ: "A" | "B" | "C" | "D"
}
...
export interface ListeningSection {
  ...
  context: string;
  instructions: string;
  script: string;    // Read ONCE via TTS
  questions: ListeningQuestion[]; // Exactly 10
}
...
export interface ReadingPassage {
  ...
  text: string;
  instructions?: string;
  questions: ReadingQuestion[]; // 13–14 per passage
}
...
// Section 1: 2–3 short everyday/social texts (~14 questions)
// Section 2: 2 work-related texts (~13 questions)
// Section 3: 1 longer general-interest article (~13 questions)
...
export interface GTReadingSection {
  sectionNumber: 1 | 2 | 3;
  label: string;            // e.g. "Section 1 – Everyday Texts"
  context: string;          // brief description
  instructions: string;
  texts: GTText[];          // 1–3 short texts
...
export interface GTWritingTask1 {
  ...
  prompt: string;
  bulletPoints: string[]; // The 3 bullet points the question gives
}
...
// ─── SPEAKING ────────────────────────────────────────────────
// Identical structure for Academic and General Training.
...
/** Official Cambridge raw-score → IELTS band conversion */
...
export function writingBandEstimate(
  ...
  const penalty = (w1Words < w1Min ? 0.5 : 0) + (w2Words < w2Min ? 0.5 : 0);
