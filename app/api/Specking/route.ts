...
export async function POST(req: NextRequest) {
  ...
  const { task, response, part, topic } = body;
  ...
  const sysPrompt = `You are an IELTS Speaking examiner. Grade the spoken response (transcript) using band descriptors (0-9). Return JSON:
{
  "overall": number,
  "ta": { "score": number, "feedback": "2-3 sentences (Fluency & Coherence)" },
  "cc": { "score": number, "feedback": "2-3 sentences (Lexical Resource)" },
  "lr": { "score": number, "feedback": "2-3 sentences (Grammatical Range)" },
  "gr": { "score": number, "feedback": "2-3 sentences (Pronunciation)" },
  "summary": "one sentence",
  "top_fix": "single actionable improvement"
}`;

  const userPrompt = `Part ${part || 2}, Topic: ${topic || 'General'}\nTask: ${task}\nTranscript: ${response}`;
