...
export async function POST(req: NextRequest) {
  ...
  // Expect new fields: testType, taskType, task, response
  ...
  if (!apiKey) {
    console.error('[ielts] DEEPSEEK_API_KEY is not set')
    return NextResponse.json({ error: 'Server configuration error.' }, { status: 500 })
  }

  const wordCount = studentResponse.trim().split(/\s+/).length

  // ----- Build task‑type specific instructions for the AI -----
...
- Organise paragraphs around each question part.`
  ...
  // ----- System prompt with anti‑cheating and task relevance rules -----
  const systemPrompt = `You are an expert IELTS examiner. You MUST follow these rules strictly:

1. **Task Relevance (CRITICAL)**: First, determine if the student's response actually addresses the given task. If the task asks for a chart description but the student writes an opinion essay, or if the response is completely off‑topic:
   - Task Achievement = 1 or 2
   - Overall band = below 3
   - Feedback must clearly say: "Your response does not address the task. You were asked to [summarise a chart / write a letter / give your opinion] but you wrote about [unrelated topic]."

2. **Repetition & Quality Check**: If the response contains repeated paragraphs (same text pasted multiple times), nonsense text, or is clearly not a genuine attempt, score 1 across all criteria and explain why.
...
Return ONLY valid JSON as specified.`
  ...
  try {
    ...
    if (!deepseekRes.ok) {
      ...
      console.error(`[ielts] DeepSeek error ${deepseekRes.status}:`, errorText)
    ...
    if (!rawContent) {
      console.error('[ielts] No content in DeepSeek response', deepseekData)
    ...
    try {
      ...
      console.error('[ielts] JSON parse failed. Raw output:', cleaned, err)
    ...
    console.error('[ielts] Unexpected error:', err)
