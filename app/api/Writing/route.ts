...
export async function POST(req: Request) {
  ...
  if (typeof task !== 'string' || task.length < 10 || task.length > 2000)
    return json({ error: 'Task must be between 10 and 2000 characters.' }, 400);

  if (typeof studentResponse !== 'string' || studentResponse.length < 50 || studentResponse.length > 10000)
    return json({ error: 'Response must be between 50 and 10,000 characters.' }, 400);
  ...
  if (testType === 'Academic' && taskType.startsWith('t1')) {
    if (
      taskType.includes('bar-chart') ||
      taskType.includes('line-graph') ||
      taskType.includes('pie-chart') ||
      taskType.includes('table')
    ) {
